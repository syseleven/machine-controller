...
func (p *provider) getConfig(provSpec clusterv1alpha1.ProviderSpec) (*Config, *providerconfigtypes.Config, *openstacktypes.RawConfig, error) {
	...
	cfg.OpenstackClientTimeout, err = p.configVarResolver.GetConfigVarDurationValueOrEnvOrDefault(rawConfig.OpenstackClientTimeout, "OS_MC_CLIENT_TIMEOUT", defaultOpenstackClientTimeout)
	if err != nil {
		return nil, nil, nil, fmt.Errorf(`failed to get the value of "OpenstackClientTimeout" field, error = %v`, err)
	}

	cfg.OpenstackComputeClientTimeout, err = p.configVarResolver.GetConfigVarDurationValueOrEnvOrDefault(rawConfig.OpenstackComputeClientTimeout, "OS_MC_COMPUTE_CLIENT_TIMEOUT", cfg.OpenstackClientTimeout)
	if err != nil {
